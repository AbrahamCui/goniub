package com.rocket.chrome.mitm;

import com.rocket.request.HttpsProxy;
import com.rocket.util.IOUtils;
...
@Slf4j
public class RocketChromeDriver extends ChromeDriver {
	
	public static final Random RANDOM = new Random();
	
	private RocketChromeOptions options;
	...
	public RocketChromeDriver() {
		this(new RocketChromeOptions());
	}
	

	public RocketChromeDriver(RocketChromeOptions options) {
		super(options);
	...
	public static String readStealthMinJs() {
		String classPathJs = "com/rocket/chrome/mitm/stealth.min.js";
		try {
			URL fileURL = RocketChromeDriver.class.getResource(classPathJs);
	...
	public static RocketChromeDriver newChromeInstance(boolean disableLoadImage, boolean headless, HttpsProxy httpsProxy) {
		return new RocketChromeDriver(new RocketChromeOptions(disableLoadImage, headless,
				httpsProxy, RocketChromeOptions.CHROME_USER_AGENT));
	}

	public static RocketChromeDriver newAndroidInstance(boolean disableLoadImage, boolean headless, HttpsProxy httpsProxy) {
		return new RocketChromeDriver(new RocketChromeOptions(disableLoadImage, headless,
				httpsProxy, RocketChromeOptions.ANDROID_USER_AGENT,true));
	}

	public static RocketChromeDriver newIOSInstance(boolean disableLoadImage, boolean headless, HttpsProxy httpsProxy) {
		return new RocketChromeDriver(new RocketChromeOptions(disableLoadImage, headless,
				httpsProxy, RocketChromeOptions.IOS_USER_AGENT));
	}
	
	public static RocketChromeDriver newInstance(boolean disableLoadImage, boolean headless, boolean hideFingerprint, HttpsProxy httpsProxy, String userAgent) {
		return new RocketChromeDriver(new RocketChromeOptions(disableLoadImage, headless, hideFingerprint,
